...
import { ShieldCheck, ClipboardCheck, Factory, Mail, CheckCircle2, AlertTriangle, FileText, BadgeCheck, ArrowRight, Search, Users, FlaskConical, CalendarCheck, TrendingUp, Leaf, Award } from "lucide-react";
import { motion } from "framer-motion";

const services = [
  { icon: ClipboardCheck, title: "Food Safety System Development", description: "Build or strengthen food safety programs to meet regulatory, customer, and audit requirements." },
  { icon: Search, title: "Internal Audits & Gap Assessments", description: "Identify gaps, evaluate risk, and create practical corrective action plans to stay audit-ready." },
  { icon: ShieldCheck, title: "Regulatory Compliance & SQF Support", description: "SQF Code implementation, documentation, supplier management, CAPA, and audit readiness support." },
...
const packages = [
  ...
  { title: "SQF / Documentation Cleanup", price: "Starting at $95/hr", description: "Hands-on support cleaning up policies, forms, corrective actions, verification records, and audit-response documentation.", includes: ["Policy and form review", "CAPA and root cause support", "Food safety plan documentation edits", "Supplier and sanitation record support"] },
];

const credentials = ["PCQI — Preventive Controls for Human Food", "HACCP — Developing and Implementing HACCP Systems", "FSPCA — Intentional Adulteration / Food Defense", "SQF Code Practitioner Training", "SQF Internal Auditing", "SQF CAPA — Corrective Action & Preventive Action", "SQF Root Cause Analysis", "SQF Risk Assessment", "SQF Supplier Management", "SQF Food Safety Culture"];

const benefits = [
  { icon: ShieldCheck, title: "Protect Your Customers", description: "Food safety protects what matters most." },
  { icon: Award, title: "Strengthen Your Brand", description: "Build trust with customers, auditors, and retailers." },
  { icon: TrendingUp, title: "Improve Efficiency", description: "Streamline systems and reduce repeat risk." },
  { icon: Leaf, title: "Sustainable Success", description: "Practical solutions for long-term results." },
];
...
function AboutPage() {
  return <main><Header />
    <section className="about-page-hero"><div className="about-page-copy"><p className="eyebrow">About Me</p><h1>Food safety consulting grounded in real manufacturing experience.</h1><div className="underline" /><p>I’m Danielle, a food safety and quality professional with 10+ years of experience supporting food safety systems in confectionery, dairy, bakery, and meat processing environments.</p><p>I specialize in SQF, HACCP, GMP, CAPA, allergen control, environmental monitoring, supplier compliance, sanitation, documentation systems, and audit readiness.</p><p>My approach is practical, collaborative, and focused on helping small manufacturers build sustainable food safety systems that work in real production environments — not just on paper.</p></div><div className="about-page-image-wrap"><img src="/danielle-headshot.jpeg" alt="Danielle" className="headshot-circle large" /></div></section>
    <section className="about-proof-strip"><article><Award /><div><h3>10+ Years</h3><p>Food safety and quality systems experience across diverse manufacturing environments.</p></div></article><article><ClipboardCheck /><div><h3>Proven Results</h3><p>Experience supporting excellent SQF audit outcomes and practical audit readiness systems.</p></div></article><article><Users /><div><h3>Practical Approach</h3><p>Solutions designed for the people who actually operate, document, clean, inspect, and verify.</p></div></article><article><ShieldCheck /><div><h3>Trusted Partner</h3><p>Focused on building strong systems, not just passing the next audit.</p></div></article></section>
    <section className="section about-detail"><div className="section-heading"><p className="eyebrow">Why Food Safety Bytes</p><h2>Practical systems. Clear documentation. Real operational fit.</h2><p>Small manufacturers often need support that is more realistic than generic templates and more approachable than large consulting firms. Food Safety Bytes bridges that gap with direct, practical support for audit readiness, allergen control, documentation cleanup, corrective actions, and food safety culture.</p></div></section>
    <ContactBand /><Footer /></main>;
}

function ContactBand() { return <section id="contact" className="cta-band"><div className="calendar-icon"><CalendarCheck /></div><div><h2>Let’s work together to strengthen your food safety program.</h2><p>Send a short summary of your facility, product type, current concern, and timeline.</p></div><a className="secondary-button" href="mailto:danielle@foodsafetybytes.com">Schedule a consultation <ArrowRight /></a></section>; }
function Benefits() { return <section className="benefits">{benefits.map((benefit) => <article key={benefit.title}><benefit.icon /><div><h3>{benefit.title}</h3><p>{benefit.description}</p></div></article>)}</section>; }
function Footer() { return <footer>© {new Date().getFullYear()} Food Safety Bytes | Food Safety Consulting</footer>; }
export default function App() { const path = window.location.pathname; return path === "/about" || path === "/about/" ? <AboutPage /> : <HomePage />; }
